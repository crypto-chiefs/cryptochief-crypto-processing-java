...
TRON energy and the native coin itself — out of API credits.

- `client.transactions().estimate()` — `POST /v1/transaction/estimate`, the network fee of a transaction that is neither signed nor broadcast. It takes the transfer fields of a sign request (`native` or `token`, no `url_callback`) and answers with `estimatedFee()` (the network fee in the native coin, human-readable) and `required()` (what the from-wallet must hold: `fee + value` for a `native` transfer, the fee alone for a `token` one); the `*Fiat()` twins are USD and read as an empty string when no rate is available. On TRON the answer carries a breakdown, `null` on every other network: `energyFee()`, `bandwidthFee()` and `activationFee()` add up to `estimatedFee()` as the gross cost, `energy()` is the energy the transfer needs, `feeLimit()` is the on-chain cap that would be written into the transaction, and `feeExpected()` is the cost given the wallet's current energy pool — an expectation, not a guarantee, the pool may be spent before broadcast. `activationFee()` is present only for a `native` transfer to an address that does not exist yet. A `contract` call cannot be estimated: 400 `CONTRACT_ESTIMATE_UNSUPPORTED`
- `ErrorCode.ESTIMATE_UNAVAILABLE` — HTTP 502 when the estimate infrastructure itself failed; nothing was priced and retrying is safe
- `client.energy()` — TRON energy rental, billed to API credits. `quote()` (`POST /v1/energy/quote`) is free and prices the rental against what burning TRX would cost; `rent()` (`POST /v1/energy/rent`) buys it synchronously — by the time it answers the energy is delegated or the refusal reason is known; `order(key)` (`POST /v1/energy/order`) reads an order back by its idempotency key. `rent()` requires an `Idempotency-Key` (call it on `client.withIdempotencyKey(key)`) and refuses to go out without one — the key is what makes a retry after a timeout safe instead of a double purchase
- `client.nativeCoin()` — `native` is a Java keyword — buys the native coin of a network (TRX, ETH, BNB, SOL, TON, ...) out of the platform's liquidity, delivered to any address, billed to API credits. `quote()` (`POST /v1/native/quote`) breaks the price down field by field — `totalUsd()` is the full price, transfer fee included, and `credits()` the exact amount the buy takes from the balance; `buy()` (`POST /v1/native/buy`) is synchronous and takes the same idempotency-key discipline as `rent()`; `order(key)` (`POST /v1/native/order`) reads an order back. A quote lives about 90 seconds and is single-use: 409 `QUOTE_EXPIRED` / `QUOTE_ALREADY_USED` means quote again
- `rent()` and `buy()` return the order even when the HTTP status is not 200: a `refused` order (502, or 402 when the credits balance did not cover it) and an `unresolved` one (409, `needsAttention()` set) arrive with the order itself as the body — a business outcome, not a transport failure — and come back as the order, not as an exception. Branch on `status()` and `needsAttention()`, and on `errorCode()` for the machine reason (`error()` is the sanitised human sentence, for logs only). Only a failure with no order to report (a 409 `NOT_WORTH_RENTING` / `QUOTE_EXPIRED` envelope, a gateway error page) throws `ApiException`. A `refused` order was never charged, so its money fields are `null` rather than zero; an `unresolved` one may already be delivered — do NOT retry it, poll `order(key)` until it settles; a `refused` one may be retried with a NEW idempotency key
- `HttpTransport` reads the machine code from an order body too: when the JSON carries `error_code` (the shape energy and native orders report failures in), `ApiException.code()` is that code and `error` is lifted into `description()` — so a body that reaches the throw, or is read through `client.request()`, still surfaces the code to branch on
- CI-hygiene: `.gitattributes` pins text checkouts to LF on every platform (`*.bat` stays CRLF) — a CRLF checkout on Windows changed the bytes of the sha256-pinned signature test vectors and failed the suite for a clean clone; GitHub Actions CI runs the suite on push
